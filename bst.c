...
{
	while(cur->left != 0)       //Continue searching until leftmost child
        cur = cur->left;

    return cur->val;      //Value of leftmost child
}
...
struct Node *_removeLeftMost(struct Node *cur)
{
    ...
    while(currNode->left != 0){     
        lastNode = currNode;         //Parent of currNode
