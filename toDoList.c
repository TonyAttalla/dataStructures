...
void printList(DynArr *heap)
{
    ...
    assert(heap != 0);      //Assert pre-condition

    sortHeap(heap);     //Sorts array into descending priority
    ...
    unSortHeap(heap);   //Re-sorts the array into a heap
}
...
void deleteList(DynArr *heap)
{
	...
	{
	  	/* get the task */
		task = (struct Task *) getMinHeap(heap);
		/* remove the task */
		removeMinHeap(heap);
		/* free the task */
		free(task);
...
int compare(TYPE left, TYPE right)
{
    ...
    node1 = (struct Task *) left;
    node2 = (struct Task *) right;
...
void print_type(TYPE curval)
{
	...
	data1 = (struct Task *) curval;

	printf("%s (%d)\n\n", data1->description, data1->priority);
}