#include<stdio.h>
#include<stdlib.h>
//#include<conio.h>   	//For getch() on Visual Studio
#include<curses.h>	//For getch() on FLIP
#include "bst.h"
#include "structs.h"
...
void testAddNode() {
	...
	myData4.number = 10;
	myData4.name = "lefty of lefty";    //Changed to lefty of lefty - RK
	addBSTree(tree, &myData4);
...
void testRemoveLeftMost() {
    ...
    printTestResult(cur == NULL, "_removeLeftMost", "removing leftmost of right of root 1st try");
 	...
 	cur = _removeLeftMost(tree->root);
    printTestResult(cur == tree->root, "_removeLeftMost", "removing leftmost of root 2nd try"); //Changed to 2nd try
}
