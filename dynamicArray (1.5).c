...
void _adjustHeap(DynArr *heap, int max, int pos)
{
    ...
    if (rightChild <= max) { /* we have two children */
        smallerChild = _smallerIndexHeap(heap, leftChild, rightChild);
        if (compare(getDynArr(heap, smallerChild), getDynArr(heap, pos)) == -1){
            swapDynArr(heap, smallerChild, pos);  //swap with smallest child 
        ...
        if (compare(getDynArr(heap, leftChild), getDynArr(heap, pos)) == -1){
            swapDynArr(heap, leftChild, pos);  //swap with smallest child
...
{
    int currIndx = (sizeDynArr(heap) / 2) - 1;

    assert(heap != 0);      //Ensure pre-condition is met

    while(currIndx >= 0){
        _adjustHeap(heap, sizeDynArr(heap) - 1, currIndx);
        currIndx--;
    }
}
/*
...
{
    int currMax = sizeDynArr(heap) - 1;

    assert(heap != 0);      //Ensure pre-condition is met
    
    _buildHeap(heap);       //Ensure binary tree is also a heap

    while(currMax >= 0){
        swapDynArr(heap, 0, currMax);
        currMax--;
        _adjustHeap(heap, currMax, 0);    
    }
}

/*
    In-place unsort of the heap
    NOTE: This function relies on implicit declaration.
          We are not permitted to change the header 
          files for this assignment, but there is no
          other means to access _buildHeap except 
          through this method.
    param: heap     pointer to the sorted array
    pre: heap is not empty
    post: the dynArr is a heap array
*/
void unSortHeap(DynArr *heap){
    _buildHeap(heap);
}